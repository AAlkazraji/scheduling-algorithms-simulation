...
public class NarrowRoundRobin {
    ...
    public NarrowRoundRobin(ArrayList<Process> newList, int DISP)
    {
    ...
    // running the execution for time quantum and check for the new arrived process every second
    public void NRR() {
        ...
        while(completeProcesses != processes.size()){
            ...
            if(runningProcess == null) {
                ...
                // when the execution time reaches time quantum
                // the running process will be checked if it has finished
                // if it finished then added to the completed queue and check if the read queue
                // still has more ready processes, if so call switchProcess() or increase the time by 1
                if(currentExecTime == runningProcess.getTimeQuantum()) {
                    if (runningProcess.isCompleted() && readyQueue.size() > 0){ // switch
                        time+=1;
                        markProcessAsDone(runningProcess);
                        runningProcess.setTimeQuantum(runningProcess.getTimeQuantum() -1);
                        runningProcess = switchProcess();
                        isSwitch = true;
                        currentExecTime = 0;
                    } else if(readyQueue.size() > 0) {  //switch
